...
namespace PLC_SIEMENS.Windows.Devices
{
    public partial class Wb1 : Form
    {
        ...
        {
            await PLC.writeBool("DB7.DBX0.1", true);
        }

        private async void stop_button_Click(object sender, EventArgs e)
        {
            await PLC.writeBool("DB7.DBX2.1", true);
        }

        private async void cycle_stany_Tick(object sender, EventArgs e)
        {
            bool bit_on = await PLC.readBool("DB6.DBX0.2");
            bool bit_awaria = await PLC.readBool("DB6.DBX0.3");
